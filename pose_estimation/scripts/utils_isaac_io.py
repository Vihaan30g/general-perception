...
def compute_intrinsics(cam_params):
    """
    Derive pixel-space intrinsics directly from the renderer's own projection
    matrix (`cameraProjection`), rather than back-deriving them from the
    physical aperture/focal-length values.

    Why not aperture + focal length? Isaac Sim's camera "aperture fit" conform
    policy can silently rescale the *effective* FOV on one axis to match the
    render resolution's aspect ratio, without updating the reported
    `cameraAperture` value. Deriving fx/fy from `cameraAperture` directly can
    then be systematically wrong on that axis -- this was confirmed on real
    capture data: aperture-derived fy was ~23% too small vs. what the renderer
    actually used, because the physical aperture (1.37 aspect) didn't match
    the render resolution (1280x720, 1.78 aspect) and Isaac conformed
    horizontally. `cameraProjection` is the exact matrix used for rendering,
    so it has no such ambiguity.

    This assumes a symmetric frustum with no lens shift (cx, cy at the image
    center) -- true whenever cameraApertureOffset is [0, 0], which covers the
    standard case. If you ever use a shifted/offset camera, P[2,0]/P[2,1]
    in the projection matrix would need to be folded into cx/cy as well.
    """
    width, height = cam_params["renderProductResolution"]
    P = np.asarray(cam_params["cameraProjection"], dtype=np.float64).reshape(4, 4)
